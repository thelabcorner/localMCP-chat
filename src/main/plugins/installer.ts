...
export function pluginEnvironment(inherited = getDefaultEnvironment(), platform = process.platform): Record<string, string> {
  ...
  // first, then standard Node and uv user-install locations. Never execute
...
export async function runInstaller(command: string, args: string[], cwd: string): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    ...
    child.once('error', () => {
      clearTimeout(timer);
      reject(new Error(`Required runtime ${path.basename(command)} is unavailable; install it and restart localMCP-chat`));
    });
...
export async function installSource(source: PluginSource, dir: string): Promise<InstalledLaunch> {
  ...
  if (source.dependencies !== undefined) {
    if (source.kind !== 'python' || !Array.isArray(source.dependencies))
      throw new Error('Dependency pins require a Python source');
  ...
  if (source.kind === 'npm') {
    ...
    if (process.platform === 'win32') {
      ...
      try {
        ...
        throw new Error('Install the standard Node.js distribution including npm, then restart localMCP-chat');
      }
...
async function findExecutable(name: string): Promise<string> {
  ...
  throw new Error(`Install ${name} and restart localMCP-chat`);
}
