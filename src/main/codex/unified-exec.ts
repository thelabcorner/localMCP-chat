...
import {
  ...
  COMPLETED_UNIFIED_EXEC_OUTPUT_BUDGET_BYTES,
...
interface CompletedProcessEntry {
  processId: number;
  exitCode: number | null;
  failure: string | null;
  rawOutput: Buffer;
...
export interface UnifiedExecProcessManagerOptions {
  /** Maximum simultaneously live/pending OS processes. Completed sessions do not count. */
  maxLiveProcesses?: number;
  /** Memory budget for unread completed-session output. Exit metadata is retained after shedding output. */
...
export class UnifiedExecProcessManager {
  ...
  private readonly completedProcesses = new Map<number, CompletedProcessEntry>();
  ...
  private readonly completedOutputBudgetBytes: number;
  private completedOutputBytes = 0;
  private nextProcessId = 1_000 + Math.floor(Math.random() * 1_000_000);

  constructor(maxWriteStdinYieldTimeMs: number, options: UnifiedExecProcessManagerOptions = {}) {
    this.maxWriteStdinYieldTimeMs = Math.max(maxWriteStdinYieldTimeMs, MIN_EMPTY_YIELD_TIME_MS);
    this.maxLiveProcesses = Math.max(1, Math.floor(options.maxLiveProcesses ?? MAX_UNIFIED_EXEC_PROCESSES));
    this.completedOutputBudgetBytes = Math.max(
      0,
      Math.floor(options.completedOutputBudgetBytes ?? COMPLETED_UNIFIED_EXEC_OUTPUT_BUDGET_BYTES)
    );
  }

  /**
   * Allocate from the full safe-integer range instead of Codex's small random 1k..100k pool.
   * Retired unread sessions intentionally keep their ids, so a tiny random namespace eventually
   * turns collision retries into another accidental long-lived-session limit.
   */
  allocateProcessId(): number {
    for (;;) {
      const processId = this.nextProcessId;
      this.nextProcessId = processId >= Number.MAX_SAFE_INTEGER ? 1_000 : processId + 1;
      if (this.reservedProcessIds.has(processId) || this.completedProcesses.has(processId)) continue;
  ...
  async writeStdin(request: WriteStdinRequest): Promise<ExecCommandToolOutput> {
    // A caller may return much later than the process itself. Compact exited rows first so
    // completion stays readable without pinning an OS-process wrapper or consuming live capacity.
    this.reapExitedProcesses();
    const completed = this.completedProcesses.get(request.processId);
    if (completed) return this.consumeCompletedProcess(completed, request);
  ...
  backgroundState(processIds: ReadonlySet<number>): BackgroundExecState {
    ...
    for (const entry of this.completedProcesses.values()) {
      if (!processIds.has(entry.processId)) continue;
      exitedUnread.push({ processId: entry.processId, exitCode: entry.exitCode });
    }
    return {
  ...
  /**
   * Capacity is based on actual live/pending OS work, not unread completion bookkeeping.
   * Exited rows are compacted first and remain drainable through write_stdin.
   */
  private ensureProcessCapacity(requestProcessId: number): void {
    this.reapExitedProcesses();
    // Reservations participate in the hard cap, so concurrent exec_command calls cannot each
    // observe one free slot and collectively insert an extra live process.
    if (this.reservedProcessIds.size > this.maxLiveProcesses) {
      throw UnifiedExecError.createProcess(
        `too many simultaneously live terminal sessions (safety limit ${this.maxLiveProcesses}); wait for or terminate a running session before starting another`
      );
    }
  ...
  /**
   * Move exited, unread sessions out of the live process table without losing their terminal
   * result. This is intentionally synchronous: admission can reclaim stale rows before deciding
   * whether a new OS process is safe to launch.
   */
  private reapExitedProcesses(): void {
    for (const [processId, entry] of this.processes) {
      if (entry.initialExecCommandActive || !entry.process.hasExited() || !entry.process.outputClosed) continue;
      ...
      try {
        ...
        if (
          ...
          current.initialExecCommandActive ||
          !current.process.hasExited() ||
          !current.process.outputClosed
        ) {
          continue;
        }
        const output = current.process.takeBuffer();
        this.processes.delete(processId);
        this.reservedProcessIds.delete(processId);
        const rawOutput = output.toBytesWithOmissionMarker();
        const completed: CompletedProcessEntry = {
          processId,
          exitCode: current.process.exitCode(),
          failure: current.process.failureMessage(),
          rawOutput,
          totalOutputBytes: output.totalBytes(),
          outputOmittedBytes: output.omittedBytes() === 0 ? null : output.omittedBytes()
        };
        this.completedProcesses.set(processId, completed);
        this.completedOutputBytes += rawOutput.length;
      } finally {
        release();
      }
    }
    this.enforceCompletedOutputBudget();
  }

  /**
   * Bound retained bytes rather than retained session count. When the byte budget is exhausted,
   * preserve the process id + exit result and convert oldest payloads into omission metadata.
   */
  private enforceCompletedOutputBudget(): void {
    if (this.completedOutputBytes <= this.completedOutputBudgetBytes) return;
    for (const entry of this.completedProcesses.values()) {
      if (this.completedOutputBytes <= this.completedOutputBudgetBytes) break;
      if (entry.rawOutput.length === 0) continue;
      this.completedOutputBytes -= entry.rawOutput.length;
      entry.rawOutput = Buffer.alloc(0);
      entry.outputOmittedBytes = entry.totalOutputBytes === 0 ? null : entry.totalOutputBytes;
    }
  }

  private consumeCompletedProcess(
    entry: CompletedProcessEntry,
    request: WriteStdinRequest
  ): ExecCommandToolOutput {
    this.completedProcesses.delete(entry.processId);
    this.completedOutputBytes = Math.max(0, this.completedOutputBytes - entry.rawOutput.length);
    if (entry.failure !== null) throw UnifiedExecError.processFailed(entry.failure);
    return {
      ...
      originalTokenCount: approxTokensFromByteCount(entry.totalOutputBytes),
      outputOmittedBytes: entry.outputOmittedBytes
    };
  }
}
