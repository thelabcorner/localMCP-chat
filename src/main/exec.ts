...
const SECRET_ENV_KEYS = [
  ...
  // Unattended localMCP-chat deployments may source the tunnel credential from either of these.
  // They belong to the connector process only and must never leak into model-launched shells,
  // tests, git helpers, language tooling, or plugin child processes through ordinary inheritance.
  'LOCALMCP_OPENAI_API_KEY',
  'LOCALMCP_OPENAI_API_KEY_FILE'
];
...
function validateEnvironment(overrides: CommandEnvironment | undefined): void {
  ...
  for (const [key, value] of entries) {
    ...
    if (key.toUpperCase().startsWith('CLF_')) {
      throw new ExecError('Environment variable names beginning with CLF_ are reserved by localMCP-chat');
    }
