 ...
 * `…\localMCP-chat\resources\rg;` — no System32, no Git, no Node. Every
 * `spawn powershell.exe ENOENT`, every `'npm' is not recognized`, every failed `where.exe`
 * in affected child processes traced back to those four characters. The machine's own registry
