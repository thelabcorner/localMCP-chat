...
export function uniqueRootName(folderPath: string, existing: readonly Root[]): string {
  ...
  // Root cardinality must not turn into a naming failure at an arbitrary 999 collisions. Keep
  // the configured 32-character slug contract by shrinking the base as the numeric suffix grows.
  for (let i = 2; i < Number.MAX_SAFE_INTEGER; i++) {
    const suffix = `-${i}`;
    const candidate = `${base.slice(0, Math.max(1, 32 - suffix.length))}${suffix}`;
    if (!taken.has(candidate)) return candidate;
  }
  throw new SandboxError('Could not allocate a root name in the safe-integer namespace');
}
